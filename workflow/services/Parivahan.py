...
from workflow.services.redirections import handle_post_redirect
...
class Parivahan:
    ...
    def process_request(self, *args, **kwargs):
        for i in range(2):    
            try:
                ...
                print(f"✗ Connection was reset or aborted: {ce}")
                return None
    ...
    def delete_otp(self, mobile_no: str, challan_no: str) -> bool:
        ...
        print(f"Deleting OTP count for challan {self.challan_no}...")
        payload = json.dumps({
            "mobile_no": mobile_no,
    ...
    def etrans_pgi_payment_status(self, challan_no: str):
        self.challan_no = challan_no or self.challan_no
        print(f"ETrans PGI payment status {self.challan_no}...")
        payload = {
            "selection": "6",
        ...
        try:
            ...
            if "No Record Found" in response.text:
                print("✓ ETrans PGI payment status response: No Record Found")
                return None
            
            table_start = response.text.find("<table>")
            table_end = response.text.find("</table>")
            if table_start == -1 or table_end == -1:
                print(f"✗ ETrans PGI payment status response: {response.text[:100]}")
                return None
            else:
                table_body = response.text[table_start:table_end+8]
                print(f"✓ ETrans PGI payment status response: {table_body}")
            ...
            print(f"✗ Error ETrans PGI payment status: {e}")
            return None
    ...
    def get_payment_status(self, reg_no: str=None, challan_no: str=None):
        ...
        print(f"Challan payment status {self.challan_no}...")
        if self.reg_no:
        ...
        response.raise_for_status()
        result = response.json()
        print(f"✓ Challan payment status response: {result}")
        ...
        #except Exception as e:
        #    print(f"✗ Error Challan payment status: {e}")    
        #    return None
        ...
        if pmt_st_data.get("application_status") in ["R", "S"]:
            if pmt_st_data.get("op_dt"):
                try:
                    ...
                    print(f"✗ Error parsing op_dt: {e}")
        return False
    
    def call_payment_verification(self, vurl: str, encdata: str):
        print(f"Call Payment Verification for challan {self.challan_no}...")
        result = handle_post_redirect(url=vurl, data={"encData": encdata})
        print(f"✓ Payment Verification response: {result}")
        return bool(result)
    
    def verify_payment(self, pmt_st_data) -> bool:
        print(f"Verify Payment for challan {self.challan_no}...")
        payload = json.dumps({
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get("status") == 200:
                    ...
                    if vurl and encdata:
                        return self.call_payment_verification(vurl=vurl, encdata=encdata)
                        
                
            return None
            
        except Exception as e:
            print(f"✗ Error Payment Verification: {e}")    
            return None
