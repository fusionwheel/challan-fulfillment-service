...
from challan_workflow.services.fw import FWLink
#from sms.adb_manager import ADBOTPManager
from challan_workflow import config
from challan_workflow.helpers import fetch_otp
...
class PaymentLinkContext(WorkflowContext):
    ...
    def get_payment_data(self, verify_payment):
        ...
        try:
            ...
            otp_details = fetch_otp(sender="VAAHAN", body_contains="getting challan detail at eChallan")
            end_time = time.time()
            print(f" ==> OTP SMS received in {end_time - start_time:.2f} seconds")
    ...
    def set_context(self):
        ...
        while i < max_retry_count:
            i += 1
            # if already department error raised then invert verify_payment for each subsequent attempt
            is_department_error = getattr(fw_service, "is_department_error", False)
            if is_department_error:
                verify_payment = 0 if verify_payment == 2 else 2 # invert verify_payment
            
            print(f"{i+1} / {max_retry_count}. Payment Link Generation attempting {self.challan_no} with verify_payment {verify_payment}")
            ...
            except exception.DepartmentError as e:
                print(f"{i+1} / {max_retry_count}. Department Error raised after {i+1} attempts attempting with verify_payment for {self.challan_no} => {verify_payment}")
                max_retry_count = 5
                #if hasattr(fw_service, "is_department_error"):
                #    verify_payment = 0 if verify_payment == 2 else 2 # invert verify_payment
                if i == max_retry_count - 1:
                    raise e
        ...
        if "payment_url" in payment_data:
            # TB Challan LINK ALLOWED EX: https://echallan.parivahan.gov.in/api/sbilanding
            if "sbi" in payment_data.get("payment_url"):    
                print(f"{i+1} / {max_retry_count}. Payment Link Generation completed !")
            elif "challan" not in payment_data.get("payment_url"):
                print(f"{i+1} / {max_retry_count}. Payment Link Generation completed !")
            else:
                raise exception.PaymentLinkOfflineChallanError("Payment Link Generation failed => Offline Challan !")
        else:
