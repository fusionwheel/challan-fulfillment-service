...
class IciciTransactionPage(BasePage):
    ...
    def add_remarks_and_proceed(self):
        ...
        # 99999 padded to remarks
        self.payment_remarks = "99999" + self.payment_remarks
...
class IciciTransactionOTPPage(BasePage):
    ...
    def enter_otp_and_submit(self):
        ...
        otp_details = fetch_otp(sender="ICICI", body_contains="from ICICI Bank Acc")
        end_time = time.time()
        print(f"ICIC OTP fetched in {end_time - start_time} seconds")
        if not otp_details:
    ...
    def click_wait_for_redirect_to_receipt(self):
        ...
        pattern = re.compile(r"data|response|transaction|success|billdesk|save", re.IGNORECASE)
    ...
    def proceed(self):
        ...
        otp_sucess = 0
        try:
            print("otp submit page url", self.page.url)
            
            self.enter_otp_and_submit()
            # checkpoint if otp submitted successfully
            ...
            print("otp submitted successfully", self.page.url)
            
            try:
                body_text = self.page.locator("body").inner_text()
                print("otp submitted => body text", body_text)
            except Exception as e:
                print("otp submitted => body text", e)
        ...
        try:
            ...
            print("receipt redirect failed = URL", self.page.url)
